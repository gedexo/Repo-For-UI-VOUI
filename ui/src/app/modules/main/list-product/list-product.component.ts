...
export class ListProductComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.activatedRoute.queryParams.subscribe(params => {
      ...
      this.listBeautyService.getDataofMainCategory(this.sectionId).subscribe(data =>{
        this.sectionName = data.name;
        this.section_title = data.name;
        this.offerSale = data.subcategories;
        this.carousalData =data.subcategories;
        this.subCategoryName =this.offerSale[0].name;
        var centerPosition = this.offerSale[0].id;

        this.offerSale.forEach((element: any) => {
          if(element.id === centerPosition){
            this.subCategoryName = element.name;
            this.TREE_DATA = element.subsubcategories;
          }

        });
        this.setProducts(centerPosition);
         }
      )
      if(this.category === 'subCategories'){
        this.listBeautyService.getDataofSubCategory(this.categoryId).subscribe(data =>{
          console.log(data);
          this.subCategoryName =data.name;
        })
      }
      if(this.category === 'subsubCategories'){
        this.listBeautyService.getDataofSubSubCategory(this.categoryId).subscribe(data =>{
          console.log(data);
        })
      }
   })
  ...
  onIndexChange(data: SlidesOutputData) {
   ...
   var splitted =  this.activeSlides.slides[0].id.split('-');
  ...
  resetCarousal():void{
    this.offerSale = this.carousalData;
    console.log(this.offerSale );
  ...
  innerCategoryRoute(subsubCategory:number){
    console.log(subsubCategory);
    const queryParams: Params = { subsubCategory: subsubCategory };
